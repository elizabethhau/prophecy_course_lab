...
def main():
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/sol_bronze_marketing")
    registerUDFs(spark)
    
    MetricsCollector.instrument(spark = spark, pipelineId = "pipelines/sol_bronze_marketing", config = Config)(pipeline)
