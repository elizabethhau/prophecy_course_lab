...
def main():
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/sol_bronze_marketing_incremental")
    registerUDFs(spark)
    
    MetricsCollector.instrument(spark = spark, pipelineId = "pipelines/sol_bronze_marketing_incremental", config = Config)(
        pipeline
    )
